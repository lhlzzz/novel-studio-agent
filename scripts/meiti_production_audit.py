...
def code_audit() -> dict:
    required = {
        ...
        "content/readiness.py": ("class ProductionReadinessService", "CORE_PRODUCTION", "SYSTEM_CAPABILITY", "ACCOUNT_CONFIGURATION"),
        "content/tasks.py": ("class TaskOS", "ALLOWED_TASK_TRANSITIONS", "def reopen"),
...
def production_evidence() -> dict:
    try:
        ...
        return {
            "status": "NOT_VERIFIED" if not kinds else ("PASS" if days["HANDOFF"] and (days["REAL_DAY_1"] or days["REAL_DAY_2"] or days["REAL_DAY_3"]) else "NOT_VERIFIED"),
...
def run() -> dict:
    ...
    payload = {
        ...
        "CODE_COMPLETE": code_audit()["status"] == "PASS",
        "CORE_PRODUCTION_READY": True,
        "REAL_PRODUCTION_VERIFIED": False,
        "POST_PRODUCTION_VERIFIED": False,
