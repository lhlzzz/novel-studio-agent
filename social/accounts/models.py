...
ACCOUNT_TRANSITIONS = {
    ...
    "AUTHENTICATED": {"VERIFYING", "EXPIRED", "REVOKED", "BLOCKED", "DEGRADED", "REFRESHING"},
    "VERIFYING": {"VERIFIED", "AUTHENTICATED", "EXPIRED", "REVOKED", "BLOCKED", "DEGRADED"},
    "REFRESHING": {"VERIFYING", "EXPIRED", "BLOCKED", "VERIFIED"},
    "VERIFIED": {"ENABLED", "EXPIRED", "REVOKED", "BLOCKED", "DEGRADED", "AUTHENTICATED", "VERIFYING", "REFRESHING"},
    "ENABLED": {"DEGRADED", "EXPIRED", "REVOKED", "BLOCKED", "VERIFIED", "REFRESHING"},
    "DEGRADED": {"ENABLED", "EXPIRED", "REVOKED", "BLOCKED", "VERIFIED", "REFRESHING"},
    "EXPIRED": {"AUTHENTICATING", "AUTHENTICATED", "REVOKED", "BLOCKED", "REFRESHING"},
    ...
    def serialized(self) -> dict[str, Any]:
        ...
        for name in sorted(names):
            ...
            payload[name] = {
                ...
                "authorized": bool(getattr(record, "authorized", False)) if record is not None else False,
                "contract_verified": bool(getattr(record, "contract_verified", False)) if record is not None else False,
                "live_verified": bool(getattr(record, "live_verified", False)) if record is not None else False,
        ...
        if payload and all(isinstance(value, dict) for value in payload.values()):
            records = {
                name: CapabilityRecord(
                    ...
                    contract_verified=bool(item.get("contract_verified")),
    ...
    def from_claimed(cls, claimed: dict[str, bool], *, verified: bool = False, method: str = "registry_claim") -> "SocialProviderCapabilities":
        records = {
            name: CapabilityRecord(
                ...
                authorized=bool(verified and value),
                contract_verified=bool(verified and value),
                live_verified=False,
                method=method,
