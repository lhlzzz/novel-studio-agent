...
def run_once(*, adapter: Any, store: JobStore | None = None, window: str = "1h") -> list[dict[str, Any]]:
    ...
    for job in store.list_jobs(("SUBMITTED", "SCHEDULED", "PUBLISHED", "PUBLISHING")):
        ...
        if publication is None:
            continue
        raw = adapter.analytics(publication) if hasattr(adapter, 'analytics') else adapter.get_analytics(publication.resolved_provider_post_id())
        ...
        snapshots = persist_metric_snapshot(metrics, source=f"native:{window}")
