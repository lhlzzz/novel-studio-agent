...
def test_capabilities_require_verification():
    ...
    account = enable_account(SocialAccount("i", "x", "x", status="VERIFIED", capabilities=SocialProviderCapabilities(publish=True, text=True), provider_account_id="acct"))
    job = DistributionJob("j", "p", "i", ContentVariant("i", "test", metadata={"approval": "approved"}), idempotency_key="k")
...
def test_v43_doctor_keeps_unverified_live_paths_blocked():
    ...
    from creative.providers.lechuang.client import DEFAULT_VIDEO_MODEL
    ...
    i2v = LechuangAdapter().capability_status("image_to_video")
